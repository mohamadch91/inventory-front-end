...
function createData(number, name, maxpop, minpop, uppervol,undervol,m25vol,m70vol,dryvol,m25volnew,m70volnew,uppervolnew,undervolnew,dryvolnew,country,parent) {
        return {
          number, name, maxpop, minpop, uppervol,undervol,m25vol,m70vol,dryvol,m25volnew,m70volnew,uppervolnew,undervolnew,dryvolnew,country,parent
        };
      }
...
useEffect(() => {
            UserService.getLevels().then((response) => {
              ...
              for(let i=0;i<response.data.length;i++){
                row.push(
        createData(response.data[i].number, response.data[i].name, response.data[i].maxpop, response.data[i].minpop, response.data[i].uppervol,response.data[i].undervol,response.data[i].m25vol,response.data[i].m70vol,response.data[i].dryvol,response.data[i].m25volnew,response.data[i].m70volnew,response.data[i].uppervolnew,response.data[i].undervolnew,response.data[i].dryvolnew,response.data[i].country,response.data[i].parent)
    ...
    return (
      <div>
        ...
        >
          <Fade in={open}>
            <Box sx={style}>
              <Typography id="transition-modal-title" className='mt-1' variant="h6" component="h2">
                Add Level
              </Typography>
              <Typography id="transition-modal-description" sx={{ mt: 2 }}>
                <Level update={useEffect} />
              </Typography>
            </Box>
          </Fade>
        </Modal>
      ...
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        <Button sx={{marginLeft:"5%"}} onClick={handleOpen} variant="contained">Add Level</Button>
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
